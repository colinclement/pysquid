        ...
        if not len(corners):  # if path consumed corners
            ...
            while v:
                if v.y == p0:  # path closes on itself
                    ...
                    medges = meshedges(meshholefromedge(mask, v.w), *mask.shape)
...
class ResistorNetworkModel(ModelComponent):
    ...
    def __init__(
        self, mask, kernel=None, phi_offset=None, gshape=None, padding=None, **kwargs
    ):
        """
        ...
        self.deltaV = kwargs.get("deltaV", 1.0)
        ...
        for edge, n1, n2 in zip(edgepath, path[:-1], path[1:]):
            for m in self.edgeMeshes[edge]:
                if m >= self.N_meshes:  # find grid mesh in mask hole
                    medges = meshedges(
                        meshholefromedge(self.mask, edge), *self.mask.shape
                    )
                else:
