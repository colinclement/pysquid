...
class FFT(object):
    def __init__(self, shape, real=True, **kwargs):
        ...
        self.wisdomfile = kwargs.get(
            "wisdom", os.path.join(os.path.expanduser("~"), ".register_wisdom.pkl")
        )
        self._loadwisdom(self.wisdomfile)

        self.a = pyfftw.empty_aligned(shape, **self.alignargs_forward)
        self.b = pyfftw.empty_aligned(
            self.realshape if real else shape, **self.alignargs
        )
        self.fftobj = pyfftw.FFTW(self.a, self.b, **self.fftwkwargs)
        self.ifftobj = pyfftw.FFTW(
            self.b, self.a, direction="FFTW_BACKWARD", **self.fftwkwargs
        )
    ...
    def _savewisdom(self, outfile):
        if outfile is None:
            return
        if outfile:
            pickle.dump(pyfftw.export_wisdom(), open(outfile, "wb"), protocol=0)
